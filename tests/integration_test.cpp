...
#include <cstring>
  ...
  */
 TEST_F(IntegrationTest, TC_CollectBoardInfo_Success) {
  ...
  */
 TEST_F(IntegrationTest, TC_HandleBoardAlert_Success) {
  ...
  */
 TEST_F(IntegrationTest, TC_resetBoard_Success) {
  ...
  */
 TEST_F(IntegrationTest, TC_selfcheckBoard_Success) {
     // 测试本地回环地址（应该能够ping通）
 ...
 */
TEST_F(IntegrationTest, TC_CollectStackInfo_Success) {
    // 创建数据采集服务，设置较短的采集间隔以便快速测试
    collector = std::make_shared<DataCollectorService>(
        chassisRepo, stackRepo, apiClient, "192.168.6.222", 1, 120);
 ...
 */
TEST_F(IntegrationTest, TC_DeployStacks_Success) {
    // 准备部署请求参数
 ...
 */
TEST_F(IntegrationTest, TC_UndeployStacks_Success) {
    // 准备停用请求参数
    std::vector<std::string> labels;
    labels.push_back("模式1");
    labels.push_back("模式2");
 ...
 */
TEST_F(IntegrationTest, TC_SendResponse_Success) {
    // 准备测试数据：创建测试机箱和板卡
    auto chassis = TestDataGenerator::CreateTestChassis(1, "TestChassis_1");
 ...
 */
TEST_F(IntegrationTest, TC_SendTaskQueryResponse_Success) {
    // 准备测试数据：创建测试机箱和板卡
    auto chassis = TestDataGenerator::CreateTestChassis(1, "TestChassis_1");
    chassis->ResizeBoards(14);
 ...
 */
TEST_F(IntegrationTest, TC_HandleTaskStartRequest_Success) {
    // 准备任务启动请求
 ...
 */
TEST_F(IntegrationTest, TC_HandleTaskStopRequest_Success) {
    // 首先启动一个任务，然后停止它
 ...
 */
TEST_F(IntegrationTest, TC_Chassis_SaveSuccess) {
    // 创建有效的机箱对象
 ...
 */
TEST_F(IntegrationTest, TC_Stack_SaveSuccess) {
    // 创建有效的业务链路对象
...
/**
 * @brief TC-Chassis-GetAll-Success: 机箱仓储GetAll方法集成测试
 * 
 * 测试InMemoryChassisRepository的GetAll方法能够正常返回所有机箱数据
 * 参考单元测试：XKSJ_test.cpp::TC_Chassis_GetAllDataIntegrity
 */
TEST_F(IntegrationTest, TC_Chassis_GetAll_Success) {
    // 创建并保存多个不同机箱号的机箱
    ...
    // 集成测试：验证GetAll返回的数据能够被ResourceMonitorBroadcaster使用
    // 调用SendResourceMonitorResponse，验证能够从仓储中读取所有机箱数据
    uint32_t requestId = 99999;
    bool sendResult = broadcaster->SendResourceMonitorResponse(requestId);
...
/**
 * @brief TC-Stack-GetAll-Success: 业务链路仓储GetAll方法集成测试
 * 
 * 测试InMemoryStackRepository的GetAll方法能够正常返回所有业务链路数据
 * 参考单元测试：XKSJ_test.cpp::TC_Stack_GetAllDataIntegrity
 */
TEST_F(IntegrationTest, TC_Stack_GetAll_Success) {
    // 创建并保存多个不同UUID的业务链路
    ...
    // 添加任务到板卡（使用业务链路中的taskID）
    TaskStatusInfo taskInfo;
    taskInfo.taskID = "task-from-stack-1";
    taskInfo.taskStatus = 1; // 运行中
    taskInfos.push_back(taskInfo);
    board.UpdateFromApiData("Board_1", "192.168.0.101", BoardType::Computing, 0, 12.5f, 2.0f, 45.0f, fanSpeeds, taskInfos);
    ...
    // 在第一个业务链路中添加任务资源
    Service service("service-uuid-1", "Service1", 0);
    Task task("task-from-stack-1", 1);
    ResourceUsage resources;
    resources.cpuUsage = 50.0f;
    resources.memoryUsage = 60.0f;
    task.UpdateResources(resources);
    service.AddOrUpdateTask("task-from-stack-1", task);
    savedStacks[0]->AddOrUpdateService(service);
    stackRepo->Save(savedStacks[0]); // 更新业务链路
